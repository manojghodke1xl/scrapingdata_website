    ...
    <div className={`inline-flex gap-3 items-center ${className}`} onClick={onClick ? onClick : null}>
      <label className={`relative flex items-center rounded-full cursor-pointer focus:outline-none ${labelProps.className || ''}`} htmlFor={id} {...labelProps}>
        <input
          ...
          onChange={onChange}
          disabled={disabled}
          className={`before:content[''] peer relative h-4 w-4 cursor-pointer appearance-none rounded-sm border border-primary transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-12 before:w-12 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-secondary checked:bg-primary-faded checked:before:bg-secondary focus:outline-none disabled:cursor-not-allowed ${
