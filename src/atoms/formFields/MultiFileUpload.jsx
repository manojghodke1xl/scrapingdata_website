...
const MultipleFileUpload = ({ imagePreviewUrl: externalImagePreviewUrl, onUploadSuccess, id, isMultiple = false, setLoading, error }) => {
  ...
  const handleFileChange = async (event) => {
    ...
    const allowedTypes = ['image/png', 'image/jpeg', 'application/svg+xml', 'image/gif'];

    const validFiles = files.filter((file) => allowedTypes.includes(file.type));
    const invalidFiles = files.filter((file) => !allowedTypes.includes(file.type));

    if (invalidFiles.length > 0) showNotification('warn', 'Some files were skipped. Accepted file types: .png, .jpeg, .svg, .gif');
        ...
        <div className="border-2 border-primary rounded-xl text-center border-dashed p-3 w-auto">
          {imagePreviews.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {isMultiple && (
                <>
                  ...
                  <div
                    className="flex items-center justify-center h-32 border-2 border-dashed border-primary rounded-xl cursor-pointer"
            ...
            <div>
              ...
              <p className="font-normal text-xs text-primary text-center m-auto">Accepted file types: .png, .jpeg, .svg, .gif</p>
            </div>
          )}
        </div>
