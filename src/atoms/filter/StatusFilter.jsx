  ...
  return (
    <div className="dropdown-container relative">
      ...
      >
        ...
        <ul className="absolute left-0 end-0 top-11 z-40 mt-1 min-w-[100px] max-w-[300px] max-h-[200px] rounded-xl bg-white text-[12px] md:text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm overflow-y-auto custom-scrollbar">
          <div className="relative py-2 border-b border-primary">
            {statuses.map((status) => (
              <li
                key={status.id}
                className={`group relative cursor-default select-none py-2 pl-3 pr-14 text-primary hover:bg-gray-50 ${selectedCategory?.id === status.id ? 'bg-lightcyan' : ''}`}
