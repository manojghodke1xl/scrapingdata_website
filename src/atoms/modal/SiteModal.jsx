import { IoCloseSharp } from 'react-icons/io5';
import MultiSelectCheckbox from '../formFields/MultiSelectCheckBox';
import useGetAllSites from '../../hooks/useGetAllSites';
import ToggleComponent from '../formFields/ToggleComponent';

const SiteModal = ({ isOpen, setSitesModelOpen, selectedSites, setSelectedSites, onConfirm, siteToggle, setSiteToggle }) => {
  const availableSites = useGetAllSites();

  if (!isOpen) return null;
  const onCancel = () => {
    setSitesModelOpen(false);
    setSelectedSites([]);
    setSiteToggle(false);
  };

  return (
    <div className="fixed inset-0 z-10 overflow-y-auto">
      <div className="fixed inset-0 bg-gray-300 bg-opacity-75 transition-opacity" onClick={() => setSitesModelOpen(false)}></div>
      <div className="flex items-start justify-center w-full min-h-screen px-2 text-center lg:absolute lg:top-[12%]">
        <div className="bg-[#FFFFFF] rounded-2xl text-left overflow-hidden shadow-xl transform transition-all w-[95%] sm:w-[80%] md:w-[580px] px-6 2xl:px-8 py-6">
          <div className="w-full flex justify-end items-center">
            <div onClick={() => setSitesModelOpen(false)} className=" flex justify-between w-full border-b border-primary mb-4 pb-4">
              <h4 className="w-full sm:text-xl text-dark text-left ">Update</h4>
              <IoCloseSharp className="cursor-pointer" />
            </div>
          </div>

          <div className="w-full flex flex-col justify-center items-center ">
            <div className="border-b border-primary w-full">
              <MultiSelectCheckbox options={availableSites} label="Select Sites" onChange={(selected) => setSelectedSites(selected)} selected={selectedSites} />
              <ToggleComponent label={(siteToggle ? 'Add' : 'Remove') + ' Site'} isEnableState={siteToggle} setIsEnableState={setSiteToggle} />
              <br />
              <br />
              <br />
              <br />
              <br />
              <br />
              <br />
              <br />
            </div>

            <div className="flex justify-end gap-5 w-full mt-6 ">
              <button onClick={() => onCancel()} className=" w-1/4 rounded-xl border border-primary text-primary py-2 ">
                Cancel
              </button>
              <button onClick={() => onConfirm(siteToggle)} className="w-1/4 rounded-xl bg-darkblue text-white py-2">
                Confirm
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default SiteModal;
