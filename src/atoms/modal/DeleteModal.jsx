...
const DeleteModal = ({ isDeleteModalOpen, onConfirm, setDeleteModalOpen, label, message }) => {
  ...
  return (
    <div className="fixed inset-0 z-10 overflow-y-auto">
      <div className="fixed inset-0 bg-secondary bg-opacity-75 transition-opacity" onClick={() => setDeleteModalOpen((prev) => ({ ...prev, isDeleteModelOpen: false }))}></div>
      <div className="flex items-start justify-center w-full min-h-screen px-2 text-center lg:absolute lg:top-[12%]">
        <div className="bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all w-[95%] sm:w-[80%] md:w-[580px] px-6 2xl:px-8 py-6">
          <div className="w-full flex justify-end items-center">
            <div onClick={() => setDeleteModalOpen((prev) => ({ ...prev, isDeleteModelOpen: false }))} className=" flex justify-between w-full border-b border-primary mb-4 pb-4">
