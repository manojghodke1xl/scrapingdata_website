...
export const updatePartnerLogoStatusApi = async (ids, isActive) => {
  return await putMethodCall(`${import.meta.env.VITE_API_URL}/partner-logo/status`, { ids, isActive });
};

export const updatePartnerLogoSitesApi = async (pids, sids, action) => {
  return await putMethodCall(`${import.meta.env.VITE_API_URL}/partner-logo/sites`, { pids, sids, action });
};
