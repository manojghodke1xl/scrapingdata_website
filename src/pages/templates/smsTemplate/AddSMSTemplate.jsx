...
import { addSmsTemplateApi, getSmsTemplateByIdApi, updateSmsTemplateApi } from '../../../apis/templates/template-apis';
...
const AddSMSTemplate = () => {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      (async () => {
        const { status, data } = await getSmsTemplateByIdApi(id);
        if (status) {
          const { site, category, ...rest } = data.smsTemplate;
          setSMSTemplate((prev) => ({ ...prev, ...rest, site: site._id, category: category || undefined }));
  ...
  const validate = () => {
    const newErrors = {};
    if (!SMSTemplate.site) newErrors.site = 'Site is required';
    if (!SMSTemplate.name.trim()) newErrors.name = 'Name is required';
    if (!SMSTemplate.message) newErrors.message = 'Message is required';
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = { ...SMSTemplate, files: [...SMSTemplate.files, ...fileIds], fileId: files._id };
      const { status, data } = await (id ? updateSmsTemplateApi(id, payload) : addSmsTemplateApi(payload));
      if (status) {
        showNotification('success', data.message);
        navigate('/templates/sms-template-list');
      } else showNotification('warn', data);
    } catch (error) {
  ...
  const variables = templateCategories.filter((item) => item._id === SMSTemplate.category)[0]?.variableMap;
  ...
  return (
    <div className="py-8 p-4 sm:p-8 overflow-x-hidden mb-20">
      <div className="w-ful pb-8 border-b border-primary gap-y-4 gap-2 flex flex-col items-start md:flex-row lg:flex-col xl:flex-row justify-between lg:items-start md:items-end xl:items-end">
        <div>
          <span className="text-3xl font-semibold text-dark">{id ? 'Edit' : 'Add'} SMS Template</span>
        </div>
        <FormButtons to="/templates/sms-template-list" type="submit" onClick={handleSubmit} btnLebal={id ? 'Save Changes' : 'Add'} loading={isLoading} />
      </div>
          ...
          <div className="w-full">
            <div className="w-full sm:w-1/2">
              <DropDown
                ...
                selected={SMSTemplate.site}
                search={true}
                commonFunction={(e) => {
                  setSMSTemplate((prev) => ({ ...prev, site: e.name }));
          ...
          <div className="w-full">
            <div className="w-full sm:w-1/2">
              ...
              <DropDown
                ...
                selected={SMSTemplate.category}
                search={true}
                commonFunction={(e) => {
                  setSMSTemplate((prev) => ({ ...prev, category: e.name }));
