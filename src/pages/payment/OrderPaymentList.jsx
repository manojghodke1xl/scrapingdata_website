...
const OrderPaymentList = () => {
  ...
  const rows = orderPayments.map((payment) => {
    const {
      _id,
      customer,
      phoneCode,
      phone,
      payment: { channel, amount, currency },
      site,
      coupon,
      products,
      createdAt,
    ...
    const totalPrice = products?.reduce((sum, p) => sum + p?.price?.[currency], 0);
    const totalSalePrice = products?.reduce((sum, p) => sum + p?.salePrice?.[currency], 0);
  ...
  const columnConfig = [
    ...
    { id: 1, label: 'Site', key: 'site', dataKey: 'site', formatForExport: (value) => (value ? `${value?.name} (${value?.host})` : '') },
    ...
    { id: 5, label: 'Products', key: 'products', dataKey: 'products', formatForExport: (value) => (value.length > 0 ? value.map((p) => p.name).join(', ') : '') },
    { id: 6, label: 'Qty', key: 'quantity', dataKey: 'products', formatForExport: (value) => (value.length > 0 ? value.reduce((sum, p) => sum + p.quantity, 0) : '') },
    {
      id: 7,
      label: 'Channel',
      key: 'channel',
    ...
    },
    {
      ...
      formatForExport: (value, data) => {
        ...
        let totalPrice = value?.reduce((sum, p) => sum + p?.price?.[currency], 0);
    ...
    },
    {
      ...
      formatForExport: (value, data) => {
        let currency = data.payment.currency;
        let totalSalePrice = value?.reduce((sum, p) => sum + p?.salePrice?.[currency], 0);
        return totalSalePrice ? `${totalSalePrice} ${data.payment.currency}` : '';
      }
    },
    { id: 10, label: 'Amount', key: 'amount', dataKey: 'payment.amount' },
    { id: 11, label: 'Coupon Code', key: 'coupon', dataKey: 'coupon.code' },
    {
      id: 12,
      label: 'Status',
      key: 'status',
