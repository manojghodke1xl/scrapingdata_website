 ...
 * Handles admin preferences including dark mode and workspace color settings
 * Allows administrators to customize their workspace appearance
 * @returns {JSX.Element} Admin settings interface
 */
const AdminSettings = () => {
  // Color context hooks for dark mode management
  const { toggleDarkMode, isDarkMode, setIsDarkMode } = useColorContext();

  // Global context for user authentication and loading state
  const {
  ...
  // State for storing workspace color preferences
  const [workspaceColors, setWorkspaceColors] = useState({});

  /**
   * Handles saving admin preference changes
   * Updates dark mode and workspace colors in the backend
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      const payload = {
        darkMode: isDarkMode,
        workspaceColors,
        layoutSize: layoutPreference // Include current layout preference
      };
