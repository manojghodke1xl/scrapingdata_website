...
import { getFanClubById } from '../../../apis/leads/fanClub-apis';
...
const ViewFanClub = () => {
  ...
  useEffect(() => {
    setLoading(true);
    (async () => {
      const { status, data } = await getFanClubById(id);
      if (status) setAdvertisement(data.fanClub);
    ...
    <div className="py-8 p-4 sm:p-8 overflow-x-hidden mb-20">
      <div className="w-full pb-8 border-b border-primary gap-y-4 gap-2 flex flex-col items-start md:flex-row lg:flex-col xl:flex-row justify-between lg:items-start md:items-end xl:items-end">
        ...
        <div className=" w-full flex gap-4 justify-end items-end md:w-fit lg:w-full xl:w-fit">
          <Link to={'/fan-club/fan-club-list'} className="px-4 py-2 text-primary font-medium bg-inherit hover:bg-hover rounded-xl border border-primary whitespace-nowrap">
            Back
          </Link>
        </div>
      </div>
      ...
      <div className="w-full justify-center items-center border-b border-primary mt-7 pb-7 gap-y-4 gap-2 lg:items-start md:items-end xl:items-end">
        <div className="w-full sm:w-[85%] md:w-[80%] lg:w-[90%] xl:w-[74%] 2xl:w-[60%] flex flex-col gap-y-2 md:flex-row justify-evenly">
          ...
          <div className="w-full mt-5">
            ...
            <p className="text-placeholder font-normal"> {advertisement?.fanClubMessage || 'Not Present'}</p>
          </div>
        </div>
      </div>
