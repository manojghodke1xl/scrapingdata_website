...
const AddAfterSale = () => {
  ...
  const handleRedirectEdit = (channel, index) => {
    if (channel === 'email') {
      let template = formState.emailTemplate.find((template) => template._id === afterSaleDetails.followUps[index].emailTemplate);
      navigate(`/templates/edit-email-template/${template._id}`);
    }
  };
