...
export default function EditMember() {
  ...
  useEffect(() => {
    setLoading(true);
    (async () => {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/member/edit/${mid}`, {
        method: 'GET',
        headers: {
  ...
  const handleDetails = async (value) => {
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/member/detail/${mid}`, {
        method: 'PUT',
        headers: {
  ...
  const handleSpouse = async (value) => {
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/member/spouse/${mid}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: localStorage.getItem('auth'),
  ...
  const handleChildren = async (value) => {
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/member/children/${mid}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: localStorage.getItem('auth'),
